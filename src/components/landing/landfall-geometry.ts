...
/**
 * Descent phases and travels in viewport-heights, DOM-free so the speed map can audit them.
 * `fade` and `hold` are the handoff contract with other-projects.tsx (README inv. 11).
 */
...
/** Travel spans the full pin: parallax never stops while stars show (they fade at entry). */
...
/** The limb's rise (vh) over limb plus this fraction of entry. */
export const EARTH_RISE_VH = 56;
export const EARTH_RISE_ENTRY_FRACTION = 0.6;
export const EARTH_SWELL_SCALE = 1.9;

/**
 * Every slot ends at the settle boundary, so a later `at` is a faster cloud. Spaced so none
 * outruns the scroll (under 1.0x) at either budget viewport.
 */
...
/**
 * Entry planes in vh, start pose to rest, over the vista's own height of scroll. Cloud planes
 * stay under the cliff's 14vh so no sky layer outruns the rock.
 */
...
/** The bank grows so its side edges never pull into view. */
