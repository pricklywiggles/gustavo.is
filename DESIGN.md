...
Two entry shapes on one rail. Every entry is one link on the 7rem date rail: date, then Wotfard 500 title and clamped standfirst. A post with a cover adds a third column, a 14rem frame the shape of the OG card (1200:630, `rounded.lg`) beside the text, so an OG-sized cover fills it edge to edge and the width the browser renders is the width it fetches; on phones the frame spans the column (100vw minus the wrapper's 3rem) above the text. Covers within a tenth of the frame's ratio (16:9 through 2:1) fill it and lose at most 10% to the crop; anything taller or wider sits inside it, so the subject is never cut and the fetched width never falls short of the rendered one. The frame carries no tint and no hairline: a transparent cover reads as art on the earth, an opaque one as a picture on it. Hover washes the entry Pale Dune/10, brightens the type to the AA-holding steps, and eases the cover up 3% inside its clipped frame (motion-safe only).
